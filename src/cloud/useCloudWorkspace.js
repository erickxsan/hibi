...
export function useCloudWorkspace(user) {
  ...
  const flushPending = useCallback(() => {
    ...
    const task = (async () => {
      ...
      if (result.status === "conflict") {
        ...
        updateSync("conflict", "Another device changed the same record. Your local version is preserved in Recovery history.");
        return result;
      }
      ...
      if (result.workspace) {
        ...
        void captureDeviceCopy(result.workspace.state, result.workspace.revision, "cloud-sync", result.workspace.updatedAt);
      }
      setError(null);
      updateSync("saved");
      return result;
      ...
      updateSync(entries.length ? "pending" : "offline", entries.length
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (localWorkspace) {
        ...
        updateSync(statusForOutbox(queued), queued.length ? "Changes are safe on this device and waiting to sync." : "");
        setLoading(false);
      }
      ...
      try {
        ...
        if (!active) return;
        if (localWorkspace) {
          updateSync(queued.length ? "pending" : "offline", queued.length
            ? "Changes are safe on this device and will retry automatically."
  ...
  const replace = useCallback(async (state) => {
    ...
    if (previous?.state) await captureDeviceCopy(previous.state, previous.revision, "before-replace", previous.updatedAt);
    try {
      ...
      if (caught instanceof WorkspaceConflictError) {
        applyWorkspace({ state: caught.latestState, versions: {}, revision: caught.latestRevision, updatedAt: caught.latestUpdatedAt });
      }
      throw caught;
    }
  ...
  const importRecords = useCallback(async (state, metadata) => {
    const previous = workspaceRef.current;
    if (previous?.state) await captureDeviceCopy(previous.state, previous.revision, "before-import", previous.updatedAt);
    try {
      ...
      if (!imported.alreadyImported) void captureDeviceCopy(imported.state, imported.revision, "cloud-import", imported.updatedAt);
      setError(null);
      updateSync("saved");
      return imported;
    } catch (caught) {
  ...
  const restoreRecoveryPoint = useCallback(async (point) => {
    await captureDeviceCopy(workspaceRef.current?.state, revisionRef.current, "before-restore", workspaceRef.current?.updatedAt);
    try {
    ...
    workspaceRepository.subscribeToWorkspace((incoming) => {
      ...
      onStatus: (status) => {
        ...
        if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") {
          updateSync(syncStatusRef.current === "pending" ? "pending" : "offline", "Live updates are disconnected; local records remain available.");
        }
      },
      onError: () => {
        if (!disposed) updateSync(syncStatusRef.current === "pending" ? "pending" : "offline", "Live updates are disconnected; local records remain available.");
      },
    }).then((unsubscribe) => {
  ...
  } : null, [findImportJob, importRecords, listRecoveryPoints, loadRecoveryPoint, replace, restoreRecoveryPoint, save, subscribe, syncMessage, syncStatus, user.id, workspace]);

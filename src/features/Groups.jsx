  ...
  const addSchedule = () =>
    setDraft((current) => ({
      ...
      weeklySchedule: [
        ...
        { id: createStableId("slot"), dayOfWeek: 1, startTime: "16:00", durationHours: defaultHours },
      ],
    }));
  const updateSchedule = (id, patch) =>
    setDraft((current) => ({
      ...current,
      weeklySchedule: current.weeklySchedule.map((slot) => (slot.id === id ? { ...slot, ...patch } : slot)),
    }));
      ...
      <details className="optional-settings">
        ...
        <div className="optional-settings-body">
          <label className="switch-row pricing-switch">
            <input
              type="checkbox"
              checked={Number.isFinite(draft.hourlyRate)}
          ...
          {!draft.weeklySchedule.length ? (
            <Field label="Planned sessions / month">
              <Input
                type="number"
                min="0"
                value={draft.plannedSessionsPerMonth}
                onChange={(event) => setDraft({ ...draft, plannedSessionsPerMonth: Number(event.target.value) })}
              />
            </Field>
          ) : null}
        </div>
  ...
  if (tab === "students") {
    return (
      <article className="detail-card member-card span-two">
        ...
        {members.map((student) => (
          <div className="member-row" key={student.id}>
      ...
      <article className="detail-card span-two">
        ...
        {gradeRows.map((row) => (
          <div className="history-line" key={row.id}>
            <span>{row.date}</span>
            <strong>
              {row.studentName} · {row.assessment}
            </strong>
...
export default function Groups({ state, derived, actions, navigate, registerNavigationBlocker }) {
  ...
  const members = useMemo(
    () => state.students.filter((student) => (student.groupIds || []).includes(selectedId)),
    [selectedId, state.students],
  );
  const memberIds = useMemo(() => new Set(members.map((student) => student.id)), [members]);
  const classRows = derived.classLog
    .filter((row) => row.groupId === selectedId || (!row.groupId && memberIds.has(row.studentId)))
    .slice()
    .sort((left, right) => right.classDate.localeCompare(left.classDate));
  const gradeRows = derived.grades
  ...
  const filtered = state.groups.filter((item) =>
    normalizeSearchText(
      [item.name, item.subject, item.grade, item.schedule, formatWeeklySchedule(item.weeklySchedule)].join(" "),
    ).includes(needle),
  );
          ...
          <div className="member-picker">
            {state.students.map((student) => (
              <label key={student.id}>
                <input
                  type="checkbox"
                  checked={(student.groupIds || []).includes(group.id)}
                  onChange={() => toggleMember(student)}
                />
                <StudentAvatar avatarId={student.avatarId} name={student.fullName} size="tiny" decorative />
                <span>
                  <strong>{student.fullName}</strong>
  ...
  return (
    <div className="page">
      ...
      <section className="group-list">
        {filtered.map((item) => {
          const itemSummary = derived.groups.find((row) => row.id === item.id) || {};
          return (
            <button className="group-row" key={item.id} type="button" onClick={() => changeSelected(item.id)}>
              <span className="group-emblem small">{item.name.slice(0, 1).toUpperCase()}</span>

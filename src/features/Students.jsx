      ...
      <div className="page-list-tools">
        ...
        <details className="student-filter-menu">
          ...
          <div className="student-filter-panel">
            <div className="filter-panel-heading"><strong>Filter students</strong><div className="filter-panel-actions">{activeFilterCount ? <button type="button" onClick={() => { setStatus("all"); setFilters({ groupIds: [], groupMatch: "any", enrollment: [] }); }}>Clear all</button> : null}<button type="button" className="filter-panel-done" onClick={(event) => { const menu = event.currentTarget.closest("details"); if (menu) menu.open = false; }}>Done</button></div></div>
            <fieldset><legend>Status</legend><div className="filter-status-options" role="group" aria-label="Filter students by status">{[["all", "All statuses"], ["Active", "Active"], ["Inactive", "Inactive"]].map(([value, label]) => <button key={value} type="button" className={status === value ? "active" : ""} aria-pressed={status === value} onClick={() => setStatus(value)}>{label}</button>)}</div></fieldset>

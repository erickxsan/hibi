...
function formatMonth(value) {
  if (!value) return "—";
  return new Intl.DateTimeFormat(getUiLocale(), { month: "long", year: "numeric" }).format(
...
function sessionLabel(session, groupsById) {
  const groupName =
    session.groupId === INDIVIDUAL_GROUP_ID
      ? "Individual students"
...
function defaultRecordContext(state, sessions) {
  ...
  const session =
    sessions.find((item) => item.classDate === asOfDate && item.recorded) ||
...
export default function Progress({ state, derived, actions, intent, clearIntent, registerNavigationBlocker }) {
  ...
  const sessions = useMemo(
    () => buildClassSessions(classRows, derived.upcomingClasses || []),
    [classRows, derived.upcomingClasses],
  );
  ...
  const recordSnapshot = useMemo(
    () => comparableRecordSnapshot(recordContext, recordEntries),
    [recordContext, recordEntries],
  );
  ...
  const hydrateRecord = useCallback(
    (nextContext) => {
      ...
      const sessionGrades = gradeRows.filter(
        (grade) => grade.date === nextContext.classDate && roster.some((student) => student.id === grade.studentId),
      );
      ...
      const entries = Object.fromEntries(
        roster.map((student) => {
          const classRow = rawClassRows.find(
            (row) =>
  ...
  const clearScores = () => {
    setRecordEntries((current) =>
      Object.fromEntries(
        Object.entries(current).map(([studentId, entry]) => [studentId, { ...entry, score: "", feedback: "" }]),
      ),
    );
  };
  ...
  const saveRecord = async () => {
    ...
    const classRecords = recordRoster.map((student) => {
      const entry = recordEntries[student.id] || {};
      const existing =
        rawClassRows.find((row) => row.id === entry.classId) ||
        rawClassRows.find(
          (row) =>
            row.studentId === student.id &&
    ...
    const gradeRecords = assessment
      ? recordRoster.flatMap((student) => {
          const entry = recordEntries[student.id] || {};
          if (entry.score === "" && !entry.feedback?.trim() && !entry.gradeId) return [];
          const existing =
            gradeRows.find((row) => row.id === entry.gradeId) ||
            gradeRows.find(
              (row) =>
                row.studentId === student.id &&
...
function RecordView({
  ...
  const selectedGroupName =
    context.groupId === INDIVIDUAL_GROUP_ID
      ? "Individual students"
      : groupsById.get(context.groupId)?.name || "Choose group";
  return (
    <section className="progress-view" role="tabpanel">
      <div className="record-class-picker">
        <div className="record-class-icon">
        ...
        <label>
          ...
          >
            {!sessions.some((session) => session.key === currentKey) ? (
              <option value="custom">
                {formatDate(context.classDate)} · {selectedGroupName}
              </option>
            ) : null}
        ...
        <div className="record-details" aria-label="Class details">
          ...
          <Field label="Hours" required>
            <Input
              ...
              step="0.25"
              value={context.hours}
              onChange={(event) => onAssessmentChange({ hours: event.target.value })}
            />
          </Field>
        </div>
      ) : null}
      ...
      <div className="record-grade-settings">
        ...
        <Field label="Maximum">
          <Input
            ...
            onChange={(event) => onAssessmentChange({ maximum: event.target.value })}
          />
        </Field>
      </div>
            ...
            {roster.map((student) => {
              ...
              return (
                <div className="progress-student-row" role="row" key={student.id}>
                  <div className="progress-student" role="rowheader">
                    ...
                    {ATTENDANCE_CODES.map((code) => {
                      ...
                      return (
                        <button
                          ...
                          title={label}
                          aria-label={label}
                          aria-pressed={entry.attendance === code}
                          className={`${attendanceTone(code)} ${entry.attendance === code ? "active" : ""}`}
                          onClick={() => onEntryChange(student.id, { attendance: code })}
                        >
                          {short}
                        </button>
                      );
                  ...
                  <label className="score-input">
                    ...
                    <Input
                      ...
                      inputMode="decimal"
                      value={entry.score}
                      onChange={(event) => onEntryChange(student.id, { score: event.target.value })}
                    />
            ...
            <dl className="summary-counts">
              ...
              {ATTENDANCE_CODES.map((code) => {
                ...
                return (
                  <div key={code}>
                    <dt>
                      <i className={attendanceTone(code)} />
                      {label}
                    </dt>
                    <dd>{summary.attendance[code]}</dd>
                  </div>
                );
              })}
            </dl>
...
function GradebookView({ state, groups, students, studentsById, gradeRows, actions }) {
  ...
  const months = useMemo(
    () => [...new Set([defaultMonth, ...gradeRows.map((row) => monthKey(row.date)).filter(Boolean)])].sort().reverse(),
    [defaultMonth, gradeRows],
  );
  const groupStudents = useMemo(
    () => students.filter((student) => student.status === "Active" && studentBelongsTo(student, filters.groupId)),
    [filters.groupId, students],
  );
  const needle = normalizeSearchText(filters.search);
  const visibleStudents = useMemo(
    () => groupStudents.filter((student) => !needle || normalizeSearchText(student.fullName).includes(needle)),
    [groupStudents, needle],
  );
  const visibleGrades = useMemo(
    () =>
      gradeRows.filter(
        (row) => monthKey(row.date) === filters.month && groupStudents.some((student) => student.id === row.studentId),
      ),
    [filters.month, gradeRows, groupStudents],
  );
  const assessments = useMemo(() => buildAssessments(visibleGrades).slice(0, 8), [visibleGrades]);
  const gradeMap = useMemo(
    () => new Map(visibleGrades.map((row) => [`${assessmentKey(row)}|${row.studentId}`, row])),
    [visibleGrades],
  );

  const createBatch = () => {
    const next = {
      date: state.settings?.asOfDate || new Date().toISOString().slice(0, 10),
      groupId: filters.groupId === "all" ? defaultGroup : filters.groupId,
      assessment: "",
      category: "Quiz",
      maximum: 20,
      entries: {},
  ...
  const batchRoster = batchDraft
    ? students.filter((student) => student.status === "Active" && studentBelongsTo(student, batchDraft.groupId))
    : [];
  const updateBatchEntry = (studentId, value) =>
    setBatchDraft((current) => ({ ...current, entries: { ...current.entries, [studentId]: value } }));
  ...
  const saveEdit = async (event) => {
    ...
    try {
      if (
        await actions.upsertGrade({
          ...editDraft,
          score: editDraft.score === "" ? null : Number(editDraft.score),
          maximum: Number(editDraft.maximum),
        })
      ) {
        setEditDraft(null);
        editBaselineRef.current = null;
              ...
              <tr>
                ...
                {assessments.map((assessment) => (
                  <th key={assessment.key} scope="col">
                    ...
                    <small>
                      {formatDate(assessment.date)} · /{assessment.maximum}
                    </small>
                  </th>
                ))}
                ...
                return (
                  <tr key={student.id}>
                    ...
                    {assessments.map((assessment) => {
                      ...
                      const value = Object.hasOwn(cellDrafts, cellKey)
                        ? cellDrafts[cellKey]
                        : compactGradeValue(grade?.score);
                      const tone =
                        grade?.percentage == null
                          ? ""
                          : grade.percentage < state.settings.lowGradeThreshold
                            ? "low"
                            : grade.percentage >= 0.9
                              ? "high"
                              : "mid";
                      return (
                        <td key={assessment.key} className={tone}>
                          <div className="grade-cell">
                            <input
                              ...
                              value={value}
                              onChange={(event) =>
                                setCellDrafts((current) => ({ ...current, [cellKey]: event.target.value }))
                              }
                              onBlur={(event) => saveCell(student, assessment, event.target.value)}
                              onKeyDown={(event) => {
            ...
            <div className="form-grid two-columns">
              ...
              <Field label="Maximum" required>
                <Input
                  ...
                  min="0.01"
                  step="0.01"
                  value={batchDraft.maximum}
                  onChange={(event) => setBatchDraft({ ...batchDraft, maximum: event.target.value })}
                />
              </Field>
            </div>
              ...
              {batchRoster.map((student) => (
                <label key={student.id}>
                  ...
                  <Input
                    ...
                    step="0.01"
                    value={batchDraft.entries[student.id] ?? ""}
                    onChange={(event) => updateBatchEntry(student.id, event.target.value)}
                  />
                  <small>/{batchDraft.maximum}</small>
                </label>
              ))}
            </div>
          </form>
          ...
          <form id="progress-edit-grade" className="drawer-form" onSubmit={saveEdit}>
            ...
            <div className="form-grid two-columns">
              ...
              <Field label="Maximum" required>
                <Input
                  ...
                  min="0.01"
                  step="0.01"
                  value={editDraft.maximum}
                  onChange={(event) => setEditDraft({ ...editDraft, maximum: event.target.value })}
                />
              </Field>
            </div>
            ...
            <Field label="Feedback">
              <TextArea
                ...
                onChange={(event) => setEditDraft({ ...editDraft, feedback: event.target.value })}
              />
            </Field>
          </form>
        ) : null}
...
function AttendanceView({ state, groups, students, classRows, sessions, language, onOpenSession }) {
  ...
  const months = useMemo(
    () =>
      [...new Set([defaultMonth, ...classRows.map((row) => monthKey(row.classDate)).filter(Boolean)])].sort().reverse(),
    [classRows, defaultMonth],
  );
  const visibleSessions = useMemo(
    () =>
      sessions
        .filter(
          (session) =>
            monthKey(session.classDate) === filters.month &&
            (filters.groupId === "all" || session.groupId === filters.groupId) &&
            classRows.some(
              (row) => classSessionKey(row) === session.key && row.classStatus === "Completed" && row.attendance,
            ),
        )
        .slice()
        .sort(
          (left, right) =>
            left.classDate.localeCompare(right.classDate) || left.startTime.localeCompare(right.startTime),
        ),
    [classRows, filters, sessions],
  );
  const sessionKeys = useMemo(() => new Set(visibleSessions.map((session) => session.key)), [visibleSessions]);
  const visibleClassRows = useMemo(
    () => classRows.filter((row) => sessionKeys.has(classSessionKey(row))),
    [classRows, sessionKeys],
  );
  const attendanceMap = useMemo(
    () => new Map(visibleClassRows.map((row) => [`${classSessionKey(row)}|${row.studentId}`, row.attendance])),
    [visibleClassRows],
  );
  const roster = useMemo(
    () =>
      students
        .filter((student) => student.status === "Active" && studentBelongsTo(student, filters.groupId))
        .filter((student) => {
          if (filters.status === "all") return true;
          return visibleSessions.some(
            (session) => attendanceMap.get(`${session.key}|${student.id}`) === filters.status,
          );
        }),
  ...
  const counts = useMemo(
    () =>
      Object.fromEntries(
        ATTENDANCE_CODES.map((code) => [code, visibleClassRows.filter((row) => row.attendance === code).length]),
      ),
    [visibleClassRows],
  );
  const studentStats = useMemo(
    () =>
      roster.map((student) => ({
        student,
        rate: attendanceRate(
          visibleSessions.map((session) => attendanceMap.get(`${session.key}|${student.id}`)).filter(Boolean),
        ),
      })),
    [attendanceMap, roster, visibleSessions],
  );
    ...
    <section className="progress-view" role="tabpanel">
      ...
      <div className="attendance-overview">
        {ATTENDANCE_CODES.map((code) => {
          const [short, label] = attendanceMeta(code, language);
          return (
            <div key={code} className={attendanceTone(code)}>
              <span>{short}</span>
              <small>{label}</small>
      ...
      {visibleSessions.length && roster.length ? (
        <div className="attendance-shell" role="region" aria-label="Monthly attendance" tabIndex="0">
          <table className="attendance-table">
            ...
            <tbody>
              {studentStats.map(({ student, rate }) => (
                <tr key={student.id}>
                  <th className="attendance-student-column" scope="row">
                    <StudentAvatar avatarId={student.avatarId} name={student.fullName} size="tiny" decorative />
                    <span>
                      <strong>{student.fullName}</strong>
                      <small>{student.code || "Student"}</small>
                  ...
                  {visibleSessions.map((session) => {
                    const code = attendanceMap.get(`${session.key}|${student.id}`);
                    const [short, label] = attendanceMeta(code, language);
                    return (
                      <td key={session.key}>
                        <span className={`attendance-dot ${attendanceTone(code)}`} title={label}>
                          {short}
                        </span>
                      </td>
                    );
      ...
      {insight ? (
        <button
          className="attendance-insight"
          type="button"
          onClick={() => {
            const session = [...visibleSessions]
              .reverse()
              .find((item) => attendanceMap.get(`${item.key}|${insight.student.id}`));

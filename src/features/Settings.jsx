...
export default function Settings({ state, actions, persistenceMode, registerNavigationBlocker }) {
  ...
  return (
    <div className="page settings-page">
      ...
      <Drawer open={recoveryOpen} onClose={() => setRecoveryOpen(false)} title="Recovery history" description="Recent server snapshots and encrypted copies kept independently on this device. Opening this history also verifies that device copies can be decrypted. Restoring never deletes the current version; it is archived first." size="normal">
        {recoveryLoading ? <p>Loading recovery history…</p> : recoveryPoints.length ? <div className="recovery-list">{recoveryPoints.map((point) => <article className="recovery-row" key={`${point.source}:${point.id}`}><div><strong>{new Date(point.capturedAt).toLocaleString()}</strong><small>{point.source === "cloud-snapshot" ? "Cloud snapshot" : "Encrypted device copy"}{point.revision !== null && point.revision !== undefined ? ` · revision ${point.revision}` : ""}{point.counts ? ` · ${point.counts.students} students, ${point.counts.classes} classes` : ""}</small></div><div className="button-cluster"><Button icon={Download} onClick={() => actions.exportRecoveryPoint(point)}>Download</Button><Button icon={RotateCcw} onClick={() => setPendingRecovery(point)}>Restore</Button></div></article>)}</div> : <p>No recovery copies are available yet. Hibi creates them automatically as you use the app.</p>}
      </Drawer>

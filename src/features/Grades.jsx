...
export default function Grades({ state, derived, actions, intent, clearIntent }) {
  ...
  useEffect(() => {
    ...
    setBatchDraft(createBatchDraft(state.groups?.[0]?.id));
    clearIntent?.();
  }, [clearIntent, intent, state.groups]);
  ...
  const saveBatch = async (event) => {
    ...
    setSaving(true);
    try {
      if (await actions.addGrades(records)) setBatchDraft(null);
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveEdit = async (event) => {
    ...
    setSaving(true);
    try {
      if (await actions.upsertGrade({ ...editDraft, score: editDraft.score === "" ? null : Number(editDraft.score), maximum: Number(editDraft.maximum) })) setEditDraft(null);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="page page-grades">
      <SectionHeading title="Grades" description="Record scores and feedback without retyping student details." actions={<Button variant="primary" icon={Plus} onClick={() => setBatchDraft(createBatchDraft(state.groups?.[0]?.id))}>Add grades</Button>} />
            ...
            <tbody>{visibleRows.map((row) => {
              ...
              return (
                <tr key={row.id}>
                  ...
                  <td><div className="row-actions"><IconButton label={`Edit ${row.assessment} for ${row.studentName}`} icon={Pencil} onClick={() => setEditDraft({ ...row, score: row.score ?? "" })} /><IconButton label={`Delete ${row.assessment} for ${row.studentName}`} icon={Trash2} onClick={() => setDeleteTarget(row)} /></div></td>
                </tr>
              );
            })}</tbody>
          </table>
        </TableShell>
      ) : <EmptyState icon={CalendarDays} title="No grades match" description="Adjust your filters or add grade records." action={<Button icon={Plus} onClick={() => setBatchDraft(createBatchDraft(state.groups?.[0]?.id))}>Add grades</Button>} />}
      ...
      <Drawer
        ...
        onClose={() => setBatchDraft(null)}
        title="Add grades"
        description="Enter one assessment for a whole roster. Blank untouched rows are skipped."
        size="wide"
        footer={<><Button onClick={() => setBatchDraft(null)} disabled={saving}>Cancel</Button><Button variant="primary" type="submit" form="batch-grade-form" disabled={saving}>{saving ? "Saving…" : "Save grades"}</Button></>}
      >
        {batchDraft ? <form id="batch-grade-form" className="drawer-form" onSubmit={saveBatch}>
          <div className="form-grid two-columns"><Field label="Date" required><Input type="date" value={batchDraft.date} onChange={(event) => setBatchDraft({ ...batchDraft, date: event.target.value })} /></Field><Field label="Group" required><Select value={batchDraft.groupId} onChange={(event) => setBatchDraft({ ...batchDraft, groupId: event.target.value, entries: {} })}><option value="">Choose group</option>{(state.groups || []).map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</Select></Field></div>
      ...
      <Drawer open={Boolean(editDraft)} onClose={() => setEditDraft(null)} title="Edit grade" footer={<><Button onClick={() => setEditDraft(null)} disabled={saving}>Cancel</Button><Button variant="primary" type="submit" form="edit-grade-form" disabled={saving}>{saving ? "Saving…" : "Save grade"}</Button></>}>
        {editDraft ? <form id="edit-grade-form" className="drawer-form" onSubmit={saveEdit}>
          ...
          <div className="form-grid two-columns"><Field label="Score"><Input type="number" min="0" step="0.01" value={editDraft.score} onChange={(event) => setEditDraft({ ...editDraft, score: event.target.value })} /></Field><Field label="Maximum" required><Input type="number" min="0.01" step="0.01" value={editDraft.maximum} onChange={(event) => setEditDraft({ ...editDraft, maximum: event.target.value })} /></Field></div>

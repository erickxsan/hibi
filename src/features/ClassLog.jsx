...
import { ATTENDANCE_CODES, CLASS_STATUSES, PAYMENT_METHODS, PAYMENT_STATUSES } from "../domain/constants";
...
function calculateClassLogRow(record, context) {
  ...
  const resolvedRate = Number.isFinite(record.appliedHourlyRate)
    ...
    : resolveHourlyRate(
        { settings: { hourlyRate }, students: student ? [student] : [], groups: group ? [group] : [] },
        student,
        group,
      );
  const charge =
    hasRequiredKey && record.classStatus
      ? roundMoney(
          record.classStatus === "Cancelled"
            ? 0
            : Number.isFinite(record.appliedCharge)
              ? record.appliedCharge
              : hours * resolvedRate,
        )
      : 0;
  ...
  if (hasRequiredKey) {
    if (!student) paymentStatus = PAYMENT_STATUSES.UNKNOWN_STUDENT;
      ...
      paymentStatus = record.classDate > asOfDate ? PAYMENT_STATUSES.SCHEDULED : PAYMENT_STATUSES.PENDING;
  ...
  const outstanding =
    !hasRequiredKey || !student || record.classDate > asOfDate || record.classStatus === "Cancelled" || charge === 0
      ? 0
...
function RosterTable({ rows, classStatus, currency, onChange, groupSelected = false, onGoToSetup }) {
  ...
  return (
    <>
      ...
      <MobileRosterCards rows={rows} classStatus={classStatus} currency={currency} onChange={onChange} />
    </>
  );
}
...
function AdvancePaymentDrawer({
  ...
  const duplicateDates = draft.entries.filter(
    (entry, index, all) => all.findIndex((item) => item.classDate === entry.classDate) !== index,
  );
  const existingDates = draft.entries.filter((entry) =>
    existingRows.some((row) => row.studentId === draft.studentId && row.classDate === entry.classDate),
  );
  const invalidEntries = draft.entries.filter(
    (entry) =>
  ...
  const updateStudent = (studentId) => {
    ...
    setDraft((current) => ({
      ...
      entries: current.entries.map((entry) => ({
        ...entry,
        amountPaid: roundMoney(effectiveNumber(entry.hours, 0) * rate),
      })),
    }));
  };
  ...
  const updateGroup = (groupId) => {
    ...
    const rate =
      resolveHourlyRate(
        { settings: { hourlyRate }, students: selectedStudent ? [selectedStudent] : [], groups: group ? [group] : [] },
        selectedStudent,
        group,
      ) ?? hourlyRate;
    setDraft((current) => ({
      ...
      <div className="drawer-form">
        <div className="form-grid form-grid-2 two-columns">
          ...
          <Field label="Group">
            ...
            >
              {selectedStudent?.isIndividual || !selectedGroupIds.length ? (
                <option value="">{t("Individual / unassigned")}</option>
              ) : null}
              {selectedGroupIds
                .map((groupId) => mapLookup(groupsById, groupId))
                .filter(Boolean)
                .map((group) => (
                  <option value={group.id} key={group.id}>
                    {group.name}
              ...
              >
                {students.map((student) => (
                  <option
                    value={student.id}
                    key={student.id}
                    data-avatar-id={student.avatarId}
                    data-meta={student.code}
                  >
                    {student.fullName}
                  </option>
                ))}
...
function HistoryView({ rows, groups, students, context, currency, actions, registerNavigationBlocker }) {
  ...
  const paymentStatuses = useMemo(
    () => [...new Set(rows.map((row) => row.paymentStatus).filter(Boolean))].sort(),
    [rows],
  );
  const filteredRows = useMemo(() => {
    ...
    return rows
      .filter((row) => {
        ...
        return [row.studentName, row.studentCode, row.groupName, row.paymentReference, row.notes].some((value) =>
                  ...
                  <tr key={row.id}>
                    ...
                    <td>
                      <div className="person-cell">
                        <StudentAvatar avatarId={row.student?.avatarId} name={row.studentName} size="tiny" decorative />
                        <span>
                          <strong>{row.studentName}</strong>
                          <span className="cell-subtitle">{row.studentCode}</span>
                        </span>
                      </div>
                    </td>
...
export default function ClassLog({
  ...
  useUnsavedChanges(
    registerNavigationBlocker,
    classDirty || advanceDirty,
    "Discard your unsaved class and payment changes?",
  );
  ...
  const changeMode = useHistoryBackedState({
    ...
    canChange: ({ from, to }) =>
      from !== "new" || from === to || confirmDiscard(classDirty, "Discard your unsaved class changes?"),
  });
  ...
  useEffect(() => {
    setRosterDrafts((current) =>
      Object.fromEntries(
        activeStudents.map((student) => [student.id, current[student.id] || makeRosterDraft(classDraft.classStatus)]),
      ),
    );
  ...
  const rosterRows = useMemo(
    () =>
      activeStudents.map((student) => {
        ...
        return {
          ...calculateClassLogRow(record, context),
          draft,
  ...
  const issues = useMemo(() => {
    const result = [];
    if (!classDraft.groupId)
      result.push({ key: "group", blocking: true, message: "Choose a group or individual students." });
    if (!isDateOnly(classDraft.classDate))
      result.push({ key: "date", blocking: true, message: "Choose a valid class date." });
    if (effectiveNumber(classDraft.hours, -1) < 0)
      result.push({ key: "hours", blocking: true, message: "Hours cannot be negative." });
    if (classDraft.groupId && !rosterRows.length)
      result.push({ key: "roster", blocking: true, message: "This group has no active students." });
    const missingAttendance = rosterRows.filter(
      (row) => classDraft.classStatus === "Completed" && !row.draft.attendance,
    ).length;
    if (missingAttendance)
      result.push({
        key: "attendance",
        blocking: true,
        message: `Choose attendance for ${missingAttendance} ${missingAttendance === 1 ? "student" : "students"}.`,
      });
    const blockingPaymentRows = rosterRows.filter((row) => BLOCKING_PAYMENT_STATUSES.has(row.paymentStatus));
    if (blockingPaymentRows.length)
      result.push({
        key: "payment",
        blocking: true,
        message: `${blockingPaymentRows.length} payment ${blockingPaymentRows.length === 1 ? "entry needs" : "entries need"} attention.`,
      });
    const duplicateCount = rosterRows.filter((row) =>
      classLogs.some(
        (saved) =>
          saved.classDate === row.classDate &&
          saved.studentId === row.studentId &&
          (saved.startTime || "") === (row.startTime || ""),
      ),
    ).length;
    if (duplicateCount)
      result.push({
        key: "duplicates",
        blocking: true,
        message: `${duplicateCount} ${duplicateCount === 1 ? "student already has" : "students already have"} a record at this date and time. Edit it in History instead.`,
      });
    ...
    if (outstandingCount)
      result.push({
        key: "outstanding",
        blocking: false,
        message: `${outstandingCount} ${outstandingCount === 1 ? "student has" : "students have"} an outstanding balance.`,
      });
    const missingMethodCount = rosterRows.filter((row) => row.amountPaid > 0 && !row.draft.paymentMethod).length;
    if (missingMethodCount)
      result.push({
        key: "method",
        blocking: false,
        message: `${missingMethodCount} paid ${missingMethodCount === 1 ? "entry has" : "entries have"} no payment method.`,
      });
    ...
    if (overpaidCount)
      result.push({
        key: "overpaid",
        blocking: false,
        message: `${overpaidCount} ${overpaidCount === 1 ? "payment is" : "payments are"} above the class charge.`,
      });
    ...
    if (lateCount)
      result.push({
        key: "late",
        blocking: false,
        message: `${lateCount} ${lateCount === 1 ? "student is" : "students are"} marked late.`,
      });
    ...
    if (excusedCount)
      result.push({
        key: "excused",
        blocking: false,
        message: `${excusedCount} ${excusedCount === 1 ? "student is" : "students are"} excused.`,
      });
    return result;
  ...
  const setRosterField = (studentId, field, value) =>
    setRosterDrafts((current) => ({
      ...current,
  ...
  const saveClass = async () => {
    ...
    try {
      ...
      if (saved) {
        const nextRosterDrafts = Object.fromEntries(
          activeStudents.map((student) => [student.id, makeRosterDraft(classDraft.classStatus)]),
        );
  ...
  const saveAdvance = async () => {
    ...
    try {
      ...
      const records = advanceDraft.entries.map((entry, index) =>
        persistedRecord({
        ...
        <div className="entry-layout">
          <div className="entry-main">
            <ClassControls
              value={classDraft}
              groups={groups}
              hasIndividualStudents={activeStudentOptions.some(
          ...
          <div className="entry-sidebar">
            <ReviewPanel
              ...
              selectedGroup={
                classDraft.groupId === UNASSIGNED_GROUP
                  ? { name: "Individual students" }
                  : mapLookup(groupsById, classDraft.groupId)
              }
              rows={rosterRows}
              summary={summary}
              issues={issues}

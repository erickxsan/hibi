...
export function canonicalGroup(draft) {
  return {
    ...
    weeklySchedule: Array.isArray(draft.weeklySchedule)
...
export function canonicalStudent(draft) {
  return {
    ...
    groupIds: Array.isArray(draft.groupIds)
      ? [...new Set(draft.groupIds.filter(Boolean))]
      : draft.groupId
        ? [draft.groupId]
        : [],
    isIndividual: Boolean(draft.isIndividual),
    customHourlyRate:
      draft.customHourlyRate === "" || draft.customHourlyRate == null ? null : Number(draft.customHourlyRate),
...
export async function persistRecipe({ baseState, recipe, adapter, replace = false, importMetadata = null }) {
  ...
  const persist = async (state) => {
    const result =
      importMetadata && adapter?.importRecords
  ...
  const next =
    adapter?.mode === "cloud" && !replace && !importMetadata ? recipe(baseState) : canonicalize(recipe(baseState));
  try {
...
export function useClassManager({ persistence } = {}) {
  ...
  const initial = useMemo(() => {
    ...
    try {
      return {
        state: importState(serializeState(persistence.initialState)),
        source: persistence.mode || "cloud",
        error: null,
      };
    } catch (error) {
      ...
      ? [
          {
            id: "storage-warning",
            tone: "error",
            message: "A browser-storage issue was detected. Your saved bytes were not overwritten automatically.",
          },
        ]
      : [],
  );
  ...
  const [syncStatus, setSyncStatus] = useState(
    persistence?.mode === "cloud" ? persistence.syncStatus || "saved" : "local",
  );
  ...
  const commit = useCallback(
    (recipe, successMessage, { replace = false, importMetadata = null } = {}) => {
      ...
      const operation = async () => {
        ...
        try {
          ...
          if (!mountedRef.current) return false;
          const saved =
            adapter?.mode === "cloud" && !replace && !importMetadata
              ? result.state
  ...
  const rawDerived = useMemo(() => deriveAll(operationalState, operationalDate), [operationalDate, operationalState]);

  const groups = useMemo(
    () => canonicalState.groups.map((group) => ({ ...group, scheduleRoom: group.schedule })),
    [canonicalState.groups],
  );
  const students = useMemo(
    () =>
      canonicalState.students.map((student) => ({
        ...student,
        studentCode: student.code,
        studentPhone: student.phone,
        importantNotes: student.notes,
      })),
    [canonicalState.students],
  );
  const gradeRows = useMemo(
    () => rawDerived.grades.map((row) => ({ ...row, maximum: row.maxScore })),
    [rawDerived.grades],
  );
  const classLogRows = rawDerived.classLog;
  const studentSummaries = useMemo(
    () =>
      rawDerived.students.map((student) => ({
        ...student,
        code: student.code,
        studentCode: student.code,
        studentPhone: student.phone,
  ...
  const groupSummaries = useMemo(
    () =>
      rawDerived.groups.map((group) => ({
        ...group,
        activeStudentCount: group.activeStudents,
        scheduleRoom: group.schedule,
      })),
    [rawDerived.groups],
  );
  ...
  const updatePreferences = useCallback(
    (patch) => {
      ...
      if (Object.hasOwn(patch, "selectedStudentId"))
        setUiPreferences((current) => ({ ...current, selectedStudentId: patch.selectedStudentId }));
      return saved;
    },
    [commit],
  );
  ...
  const upsertGroup = useCallback(
    (draft) => {
      ...
      return commit(
        ...
        draft.id ? "Group updated" : "Group added",
      );
    },
    [commit, notify],
  );
      ...
      return commit(
        (current) => ({
          ...
          scheduleExceptions: current.scheduleExceptions.filter(
            (item) => item.groupId !== id && item.sourceGroupId !== id,
          ),
  ...
  const upsertStudent = useCallback(
    (draft) => {
      ...
      return commit(
        (current) => ({
          ...current,
          students: current.students.some((student) => student.id === item.id)
            ? current.students.map((student) => (student.id === item.id ? item : student))
            : [...current.students, item],
        }),
        draft.id ? "Student updated" : "Student added",
      );
    },
    [commit, notify],
  );
  ...
  const deleteStudent = useCallback(
    (id) => {
      if (
        stateRef.current.grades.some((grade) => grade.studentId === id) ||
        stateRef.current.classLog.some((row) => row.studentId === id)
      )
        return notify("Archive this student to preserve grade and payment history.", "error");
      return commit(
        (current) => ({
          ...current,
          students: current.students.filter((student) => student.id !== id),
          classSchedules: current.classSchedules.filter(
            (item) => item.studentId !== id && !item.participantIds?.includes(id),
          ),
          scheduleExceptions: current.scheduleExceptions.filter(
            (item) => item.studentId !== id && !item.participantIds?.includes(id),
          ),
  ...
  const upsertGrade = useCallback(
    (draft) => {
      ...
      return commit(
        (current) => ({
          ...current,
          grades: current.grades.some((grade) => grade.id === item.id)
            ? current.grades.map((grade) => (grade.id === item.id ? item : grade))
            : [...current.grades, item],
        }),
        draft.id ? "Grade updated" : "Grade added",
      );
    },
    [commit, notify],
  );
  ...
  const addGrades = useCallback(
    (drafts) => {
      ...
      const error = items
        .map((item) => validateGrade(item, { ...current, grades: [...current.grades, ...items] }))
      ...
      return commit(
        (current) => ({ ...current, grades: [...current.grades, ...items] }),
        `${items.length} grade${items.length === 1 ? "" : "s"} saved`,
      );
    },
    [commit, notify],
  );
  ...
  const deleteGrade = useCallback(
    (id) =>
      commit((current) => ({ ...current, grades: current.grades.filter((grade) => grade.id !== id) }), "Grade deleted"),
    [commit],
  );

  const upsertClassLog = useCallback(
    (draft) => {
      ...
      return commit(
        (current) => ({
          ...current,
          classLog: current.classLog.some((row) => row.id === item.id)
            ? current.classLog.map((row) => (row.id === item.id ? item : row))
            : [...current.classLog, item],
        }),
        draft.id ? "Class record updated" : "Class record added",
      );
    },
    [commit, notify],
  );

  const addClassLogs = useCallback(
    (drafts) => {
      const items = drafts.map((draft) =>
        applyGeneratedId(createClassLogRow, canonicalClassLog(draft, stateRef.current)),
      );
      if (!items.length) {
      ...
      const duplicate = items.find((item) =>
        current.classLog.some(
          (row) =>
            row.studentId === item.studentId &&
            row.classDate === item.classDate &&
      ...
      return commit(
        (current) => ({ ...current, classLog: [...current.classLog, ...items] }),
        `Class saved for ${items.length} student${items.length === 1 ? "" : "s"}`,
      );
    },
    [commit, notify],
  );

  const deleteClassLog = useCallback(
    (id) =>
      commit(
        (current) => ({ ...current, classLog: current.classLog.filter((row) => row.id !== id) }),
  ...
  const saveProgress = useCallback(
    ({ classRecords = [], gradeRecords = [] } = {}) => {
      const current = stateRef.current;
      const classItems = classRecords.map((draft) =>
        applyGeneratedId(createClassLogRow, canonicalClassLog(draft, current)),
      );
      ...
      const classError = classItems
        .map((item) => validateClassLogRow(item, proposed))
        .find((validation) => !validation.valid);
      const gradeError = gradeItems
        .map((item) => validateGrade(item, proposed))
        .find((validation) => !validation.valid);
  ...
  const upsertClassSchedule = useCallback(
    (draft) => {
  ...
  const upsertScheduleException = useCallback(
    (draft) => {
  ...
  const upsertScheduleChange = useCallback(
    (draft) => {
      const item = applyGeneratedId(createScheduleChange, canonicalScheduleChange(draft));
      return commit(
        (current) => ({ ...current, scheduleChanges: [...current.scheduleChanges, item] }),
  ...
  const editScheduledClass = useCallback(
    ({ session, draft, scope, asOfDate }) =>
      commit(
  ...
  const removeScheduledClass = useCallback(
    ({ session, scope, asOfDate }) =>
      commit(
    ...
    (text) => {
      try {
        ...
        const currentTotal = [
          ...
          stateRef.current.grades,
          stateRef.current.classLog,
        ].reduce((total, items) => total + items.length, 0);
        const importedTotal = [imported.groups, imported.students, imported.grades, imported.classLog].reduce(
        ...
        return commit(
          (current) => {
            ...
            if (fresh.signature !== signature) {
              throw new Error(
                "Your records changed after the preview. Reopen the file and review the updated conflicts.",
              );
            }
  ...
  const exportRecoveryPoint = useCallback(
    async (point) => {
